...
func New(reg registry.Registry, cfg *config.EncryptionConfig) (Encryption, hcl.Diagnostics) {
	...
	if cfg.StateFile != nil {
		enc.statefile, encDiags = newStateEncryption(enc, cfg.StateFile.TargetConfig, cfg.StateFile.Enforced, "statefile")
	...
	if cfg.PlanFile != nil {
		enc.planfile, encDiags = newPlanEncryption(enc, cfg.PlanFile.TargetConfig, cfg.PlanFile.Enforced, "planfile")
		diags = append(diags, encDiags...)
	} else {
	...
	if cfg.Backend != nil {
		enc.backend, encDiags = newStateEncryption(enc, cfg.Backend.TargetConfig, cfg.Backend.Enforced, "backend")
		diags = append(diags, encDiags...)
	} else {
	...
	if cfg.Remote != nil {
		for _, remoteTarget := range cfg.Remote.Targets {
			...
			enc.remotes[remoteTarget.Name], encDiags = newStateEncryption(enc, remoteTarget.TargetConfig, false, addr)
			diags = append(diags, encDiags...)
		}
	}
	if diags.HasErrors() {
