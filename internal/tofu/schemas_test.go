...
func simpleTestSchemas() plugins.Schemas {
	return simpleMockPluginLibrary()
}
...
func schemaOnlyProvidersForTesting(schemas map[addrs.Provider]providers.ProviderSchema, t *testing.T) plugins.Manager {
	...
	for providerAddr, schema := range schemas {
		...
		factories[providerAddr] = mockPluginFactory{func() (providers.Interface, error) {
			return provider, nil
		}}
	}

	return plugins.NewManager(providers.NewManager(factories), nil)
}
