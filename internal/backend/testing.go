...
func TestBackendStates(t *testing.T, b Backend) {
	...
	noDefault := false
	if _, err := b.StateMgr(DefaultStateName, nil); err != nil {
...
func testLocksInWorkspace(t *testing.T, b1, b2 Backend, testForceUnlock bool, workspace string) {
	...
	b1StateMgr, err := b1.StateMgr(DefaultStateName, nil)
	if err != nil {
		t.Fatalf("error: %s", err)
	}
	...
	b2StateMgr, err := b2.StateMgr(DefaultStateName, nil)
	if err != nil {
		t.Fatalf("error: %s", err)
	}
	...
	_, err = b2.StateMgr(DefaultStateName, nil)
	if err != nil {
